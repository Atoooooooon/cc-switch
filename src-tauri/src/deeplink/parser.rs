...
//!
//! Parses bistrocode:// and ccswitch:// URLs into DeepLinkImportRequest structures.
...
/// Parse a bistrocode:// or ccswitch:// URL into a DeepLinkImportRequest
///
/// Expected format:
/// bistrocode://v1/import?resource={type}&...
pub fn parse_deeplink_url(url_str: &str) -> Result<DeepLinkImportRequest, AppError> {
    ...
    if scheme != "bistrocode" && scheme != "ccswitch" {
        return Err(AppError::InvalidInput(format!(
            "Invalid scheme: expected 'bistrocode' or 'ccswitch', got '{scheme}'"
        )));
    }
...
fn parse_provider_deeplink(
    ...
    if !matches!(
        app.as_str(),
        "claude" | "cursor" | "codex" | "gemini" | "opencode" | "openclaw" | "hermes"
    ) {
        return Err(AppError::InvalidInput(format!(
            "Invalid app type: must be 'claude', 'cursor', 'codex', 'gemini', 'opencode', 'openclaw', or 'hermes', got '{app}'"
        )));
    }
...
fn parse_mcp_deeplink(
    ...
    for app in apps.split(',') {
        ...
        if !matches!(
            ...
            return Err(AppError::InvalidInput(format!(
                "Invalid app in 'apps': must be 'claude', 'cursor', 'codex', 'gemini', 'opencode', 'openclaw', or 'hermes', got '{trimmed}'"
            )));
        }
    }
